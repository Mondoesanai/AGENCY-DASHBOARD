...
async function runAgentCycleInner(site, st, { manual = false } = {}) {
  ...
  // 6000 chars was enough to silently truncate BEFORE reaching the relevant
  // part of a real page — a rankings page listing bracelet 001, 002, 003...
  // could easily cut off before "003" if it's not near the top of the file,
  // meaning the model never actually saw what it was asked to change and had
  // no way to know that. Sonnet's context window has plenty of room for full
  // real-world page sizes.
  const SAMPLE_CHARS = 16000;
  const sample = {};
  for (const p of [...configish, ...likelyTargets].filter(Boolean).slice(0, 6)) {
    const c = await getFileContent(site.repo, p, tree.branch);
    if (c) sample[p] = c.slice(0, SAMPLE_CHARS);
  }
