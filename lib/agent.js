...
function buildPlaybook({ keywords, rankLine, intel, files, htmlFiles, done, homepage }) {
  const M = MONTH();
...
${intel}` : ''}`,
  ...
  for (const p of pages) {
    items.push({
      ...
      detail: `Edit ONLY markup attributes in ${p} — never image/video files, never visible design. Where clearly safe: add loading="lazy" and decoding="async" to <img> that are below the fold; mark the single main hero/LCP image with fetchpriority="high" (and no lazy loading); add width/height attributes to <img> that lack them when the rendered size is stated nearby in existing CSS/attributes (skip otherwise); add <link rel="preconnect"> for third-party origins (fonts, analytics) that the page really uses; add defer to a non-critical external <script src> only if nothing inline depends on it. Slow pages rank worse and lose visitors. If nothing here is safely improvable, reply NOOP.`,
    });
  }
  for (const p of pages) {
    items.push({
