...
async function blogState(slug) {
  ...
  try {
    ...
    return { posts: Array.isArray(o.posts) ? o.posts : [], lastAt: o.lastAt || 0, retryAt: o.retryAt || 0, unsupported: o.unsupported || '' };
  } catch {
    return { posts: [], lastAt: 0, retryAt: 0, unsupported: '' };
  }
}
...
- One H1 ("Blog" or similar, natural for this business), one short intro sentence using only facts on the homepage, then a list/grid container for posts that is EMPTY except for this exact comment on its own line: ${BLOG_MARKER}
...
---END PATCH---`;
    ...
    if (!idx || !/<h1[\s>]/i.test(idx.content) || !idx.content.includes(BLOG_MARKER) || idx.content.length < 600 || looksSpammy(idx.content)) return fail('blog home draft did not pass the checks' + (plan.patchErrors?.length ? ' (' + plan.patchErrors[0] + ')' : ''));
    const res = await commitChangeset(site.repo, {
