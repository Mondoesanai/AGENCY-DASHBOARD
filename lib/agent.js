...
async function runAgentCycleInner(site, st, { manual = false } = {}) {
  ...
  // detail template, nothing about the task's wording matched it), and a
  // pure "only include what scored" rule would have dropped it from the
  // sample entirely even though it turned out to hold the exact mechanism
  // (a live Firebase read) the task was actually about. For a small site,
  // guessing which few files matter is pure downside — just show the model
  // everything and let it decide. Only fall back to scoring/narrowing once a
  // site has enough pages that sampling all of them stops being cheap.
  const taskText = `${targetItem.title || ''} ${targetItem.detail || ''}`.toLowerCase();
  let likelyTargets;
  if (htmlFiles.length <= 8) {
    likelyTargets = htmlFiles;
  } else {
  ...
  // BEFORE reaching the relevant part of a real page — one real file here
  // was 53KB and the part that mattered (a live-database read, not static
  // content) was past the old cutoff, so the model never saw it and had no
  // way to know that. Sonnet's context window has plenty of room for full
  // real-world page sizes; the real constraint is total prompt size across
  // ALL sampled files combined, not any one file, so cap that instead.
  const PER_FILE_CAP = 100000;
  const TOTAL_SAMPLE_BUDGET = 140000;
  const sample = {};
