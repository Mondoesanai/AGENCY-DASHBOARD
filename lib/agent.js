...
import { rankBatch, serpConfigured, indexedPages } from './serp.js';
...
// Pacing is measured from the last change that actually SHIPPED, not the last
// attempt. It used to key off every cycle, so a setup-only step (picking
// keywords), a "nothing to change" answer or a killed run locked a site out for
// two days — four of six sites sat idle behind attempts that produced nothing.
// Now: ~1 day between shipped changes (a steady drip Google likes and a budget
// that holds — each change costs cents), but only ~30 minutes between attempts (failures are bounded
// separately: 3 a day, then paused).
const MIN_DISCRETIONARY_GAP_MS = 20 * 3600000; // between a site's SHIPPED changes — about one a day, so there is visible progress daily
const ATTEMPT_GAP_MS = 30 * 60000; // between attempts of any kind
...
export async function agentStatus(site) {
  ...
  const lastShipAt = Number(await store.get(`agent:lastShipAt:${site.slug}`).catch(() => 0)) || 0;
  if (lastShipAt && Date.now() - lastShipAt < MIN_DISCRETIONARY_GAP_MS) {
    const hrsLeft = Math.ceil((MIN_DISCRETIONARY_GAP_MS - (Date.now() - lastShipAt)) / 3600000);
    reasons.push(`paced — a change just shipped; spreading the work out instead of front-loading it. Next improvement in ~${hrsLeft}h`);
  }
  const lastCycleAt = Number(await store.get(`agent:lastCycleAt:${site.slug}`).catch(() => 0)) || 0;
  if (lastCycleAt && Date.now() - lastCycleAt < ATTEMPT_GAP_MS) {
    reasons.push(`paced — a cycle just ran on this site; next attempt in ~${Math.ceil((ATTEMPT_GAP_MS - (Date.now() - lastCycleAt)) / 60000)} min`);
  }
...
export function looksSpammy(text) {
  // judge the COPY, not the markup, and count each distinct phrase once: a title reused in
  // og:title and twitter:title, or a handful of <meta ... content="..."> tags, repeat words
  // like "<meta", "content=" and the brand name enough to look like keyword stuffing — that
  // silently rejected every legitimate title/description rewrite on pages with social tags
  const raw = String(text || '').replace(/<script[\s\S]*?<\/script>|<style[\s\S]*?<\/style>/gi, ' ');
  const pieces = new Set();
  for (const m of raw.matchAll(/\b(?:content|alt|title)=(["'])(.*?)\1/gi)) pieces.add(m[2].trim().toLowerCase());
  for (const seg of raw.replace(/<[^>]*>/g, '\u0000').split('\u0000')) if (seg.trim()) pieces.add(seg.trim().toLowerCase());
  const t = [...pieces].join(' ');
...
export async function initKeywordTracking(site) {
  ...
  if (serpConfigured()) {
    try {
      const [r, indexed] = await Promise.all([rankBatch({ domain: site.url, keywords: keywords.slice(0, 15), device: 'mobile', concurrency: 8 }), indexedPages(site.url)]);
      ranks = { at: Date.now(), ...r, indexed };
...
export async function refreshRanksIfStale(site, { force = false } = {}) {
  ...
  if (!force) {
    ...
    if (last && last.at && Date.now() - last.at < RANK_REFRESH_INTERVAL_MS && last.indexed !== undefined) {
      return { ok: true, skipped: true, reason: 'checked recently' };
    }
  }
  try {
    const [r, indexed] = await Promise.all([rankBatch({ domain: site.url, keywords: keywords.slice(0, 15), device: 'mobile', concurrency: 8 }), indexedPages(site.url)]);
    const ranks = { at: Date.now(), ...r, indexed };
    await saveRanks(site.slug, ranks);
...
// ---------------------------------------------------------------------------
// Weekly blog. Runs as its own lane in the agent: one useful post a week on the
// client's OWN site (blog/<slug>.html), listed on blog/index.html and in the
// sitemap. It is one shipped change like any other, so it shares the monthly
// budget (~10-20 cents a post) and never crowds out the daily fixes — the
// playbook simply continues the day after a post ships.
// ---------------------------------------------------------------------------
const BLOG_GAP_MS = 6.5 * 24 * 3600000;
const BLOG_MARKER = '<!-- NEW POSTS GO HERE -->';
const blogOn = (site) => site.blog !== false && process.env.AGENT_BLOG !== 'off';
async function blogState(slug) {
  const raw = await store.get(`agent:blog:${slug}`).catch(() => null);
  try {
    const o = raw ? (typeof raw === 'string' ? JSON.parse(raw) : raw) : {};
    return { posts: Array.isArray(o.posts) ? o.posts : [], lastAt: o.lastAt || 0, retryAt: o.retryAt || 0, unsupported: o.unsupported || '', lastError: o.lastError || '' };
  } catch {
    return { posts: [], lastAt: 0, retryAt: 0, unsupported: '', lastError: '' };
  }
}
async function saveBlogState(slug, st) {
  await store.set(`agent:blog:${slug}`, JSON.stringify({ ...st, posts: st.posts.slice(-80) }), { ex: 60 * 60 * 24 * 400 }).catch(() => {});
}
export async function blogSummary(slug) {
  const st = await blogState(slug);
  const last = st.posts[st.posts.length - 1];
  return { posts: st.posts.length, last: last ? { title: last.title, path: last.path, at: last.at } : null, nextEligibleAt: st.lastAt ? st.lastAt + BLOG_GAP_MS : 0, retryAt: st.retryAt || 0, unsupported: st.unsupported || '', lastError: st.lastError || '' };
}
export async function blogDue(site, force = false) {
  if (!blogOn(site)) return false;
  const st = await blogState(site.slug);
  if (st.unsupported) return false;
  return (force || Date.now() >= st.retryAt) && Date.now() - st.lastAt >= BLOG_GAP_MS;
}
const hasShell = (h) => h.includes('<!-- BLOG-MAIN-START -->') && h.includes('<!-- BLOG-MAIN-END -->') && /<!-- CARD-TEMPLATE:[\s\S]*?-->/.test(h) && /<!-- POST-SHELL:[\s\S]*?\{\{CONTENT\}\}[\s\S]*?-->/.test(h);
const esc = (x) => String(x).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');
const wordCount = (html) => String(html).replace(/<script[\s\S]*?<\/script>|<style[\s\S]*?<\/style>|<[^>]+>/g, ' ').split(/\s+/).filter(Boolean).length;
const plainText = (html) => String(html).replace(/<script[\s\S]*?<\/script>|<style[\s\S]*?<\/style>|<[^>]+>/g, ' ').replace(/\s+/g, ' ');

async function runBlog(site, { key, spend, tree, homepageFile }) {
  const st = await blogState(site.slug);
  const fail = async (why, { retryHours = 12, unsupported = '' } = {}) => {
    st.retryAt = Date.now() + retryHours * 3600000;
    st.lastError = String(why).slice(0, 200);
    if (unsupported) st.unsupported = unsupported;
    await saveBlogState(site.slug, st);
    return { ok: false, error: why };
  };
  const template = await getFileContent(site.repo, homepageFile, tree.branch);
  if (!template) return fail('could not read the homepage to use as a template');
  const host = String(site.url).replace(/^https?:\/\//, '').replace(/\/+$/, '');
  // a blog home from before the reusable shell existed is rebuilt (only when it has no posts to lose)
  const idxNow = tree.files.includes('blog/index.html') ? await getFileContent(site.repo, 'blog/index.html', tree.branch) : null;
  const hasIndex = !!idxNow && (hasShell(idxNow) || st.posts.length > 0);
  const sitemap = tree.files.includes('sitemap.xml') ? await getFileContent(site.repo, 'sitemap.xml', tree.branch) : null;
  const sitemapRule = sitemap ? '\nAlso add the new URL to sitemap.xml with a PATCH (FIND must be exactly `</urlset>`, which appears once; REPLACE = one <url> entry + `</urlset>`).' : '';
  const planSystem = 'You are a careful local-SEO content writer and front-end engineer adding to a real client website. You never invent facts. Reply in the exact plain-text format requested — no JSON, no markdown fences.';

  // ---- step 1 (once per site): the blog home page ----
  if (!hasIndex) {
    const user = `Business: ${site.name} — ${site.url}
The site's homepage, whose <head>, header/navigation and footer you must reuse so the blog looks like part of the site:
${template.slice(0, 50000)}
${sitemap ? `\nCurrent sitemap.xml:\n${sitemap.slice(0, 4000)}` : ''}

TASK: create the blog home page blog/index.html (there are no posts yet).
- Copy the <head> (stylesheet/font/script links, meta viewport, favicon), header/nav and footer from the homepage. Use root-absolute or correctly relative paths so assets still load from the blog/ folder. If the homepage's styling is a large inline <style> block that cannot reasonably be reused, reply ONLY: NOOP: template uses large inline styles
- Wrap the page's whole main content in the comments <!-- BLOG-MAIN-START --> and <!-- BLOG-MAIN-END -->. Inside them put: one H1 ("Blog" or similar, natural for this business), one short intro sentence using only facts on the homepage, then a list/grid container for posts that is EMPTY except for this exact comment on its own line: ${BLOG_MARKER}
- Still INSIDE the main-content comments, add two HTML comments that hold reusable markup so later posts match the site's styling exactly: <!-- CARD-TEMPLATE: one post card using the site's own classes, with the placeholders {{URL}} {{TITLE}} {{TEASER}} {{DATE}} --> and <!-- POST-SHELL: the wrapper markup (section/container/article with the site's own classes) that surrounds a post's text, with the single placeholder {{CONTENT}} --> . Do not write two hyphens in a row inside those comments.
- Unique <title> (≤60 chars), <meta name="description"> (≤155 chars), <link rel="canonical" href="https://${host}/blog/">.
- ALSO add a small "Blog" link to the homepage (${homepageFile}) via ONE PATCH: put it in the footer links (or the main nav if there are no footer links), copying the exact markup style of a neighbouring link. The FIND must appear exactly once. If you cannot do this safely, skip that PATCH.${sitemapRule}

Reply in EXACTLY this format:
SUMMARY: ONE plain-English sentence for the business owner, e.g. "Set up a blog section on your site"
COMMIT: commit message
...
---END CONTENT---
PATCH: ${homepageFile}
REASON: link to the blog
---FIND---
exact text
---REPLACE---
replacement
---END PATCH---`;
    const first = await callAgent(key, planSystem, { prefix: user }, 8000, { model: HAIKU });
    await spend(first.usd);
    const plan = await finalizePlan(first.text, { repo: site.repo, branch: tree.branch });
    if (!plan.files.length && plan.noop) return fail('blog skipped — ' + plan.noop, { retryHours: 24 * 400, unsupported: plan.noop });
    const idx = plan.files.find((f) => f.path === 'blog/index.html');
    const okFiles = plan.files.filter((f) => f.path === 'blog/index.html' || f.path === 'sitemap.xml' || (f.path === homepageFile && f.patched && /blog/i.test(f.replaceText || '') && (f.replaceText || '').length < 1500));
    if (!idx || !/<h1[\s>]/i.test(idx.content) || !idx.content.includes(BLOG_MARKER) || !hasShell(idx.content) || idx.content.length < 600 || looksSpammy(plainText(idx.content))) return fail('blog home draft did not pass the checks' + (plan.patchErrors?.length ? ' (' + plan.patchErrors[0] + ')' : ''));
    const res = await commitChangeset(site.repo, {
      files: okFiles.map((f) => ({ path: f.path, content: f.content })),
      message: (plan.commitMessage || 'Add blog section').slice(0, 100),
      branchPrefix: 'seo-blog',
      autoMerge: !!site.agentAutoMerge,
      body: `**${plan.summary || 'Set up a blog section'}**\n\n_Automated by the Inspiring Websites SEO agent._`,
    });
    await log(site.slug, { action: site.agentAutoMerge ? 'shipped' : 'PR opened', detail: 'blog section set up', prUrl: res.prUrl });
    await noteShipped(site.slug, plan.summary || 'Set up a blog section on your site so we can publish helpful articles every week');
    st.retryAt = 0; // the first post follows on the next cycle
    await saveBlogState(site.slug, st);
    return { ok: true, action: 'blog-setup', summary: plan.summary, files: okFiles.map((f) => f.path), pr: res };
  }

  // ---- step 2 (weekly): one post ----
  const raw = await store.get(`agent:ranks:${site.slug}`).catch(() => null);
  let rr = null;
  try {
    rr = raw ? (typeof raw === 'string' ? JSON.parse(raw) : raw) : null;
  } catch {
    rr = null;
  }
  const used = new Set(st.posts.map((p) => String(p.topic).toLowerCase()));
  const kws = (await readArr(`agent:keywords:${site.slug}`)).slice(0, 15);
  const ideas = [];
  for (const r of rr?.results || []) {
    if (!r.error && (!r.rank || r.rank > 10)) ideas.push(r.keyword);
    for (const q of [...(r.paa || []), ...(r.related || [])].slice(0, 6)) ideas.push(q);
  }
  ideas.push(...kws);
  const candidates = [...new Set(ideas.map((s) => String(s).trim()).filter(Boolean))].filter((s) => !used.has(s.toLowerCase())).slice(0, 14);
  const today = new Date().toISOString().slice(0, 10);
  const indexHtml = await getFileContent(site.repo, 'blog/index.html', tree.branch);
  if (!indexHtml || !indexHtml.includes(BLOG_MARKER)) return fail('blog home page lost its marker; needs a manual look', { retryHours: 24 * 7 });
  if (hasShell(indexHtml)) {
    // FAST PATH: the model writes only the article text (~1.5k tokens); the page shell —
    // head, navigation, footer, wrapper markup — is copied from the blog home page in code.
    const homeTxt = plainText(template).slice(0, 5000);
    const links = tree.files.filter((p) => /\.html?$/i.test(p) && !p.startsWith('blog/')).slice(0, 14);
    const fuser = `Business: ${site.name} — ${site.url}
Date: ${today}
What the homepage says (the ONLY source of facts about the business): ${homeTxt}
Pages on the site you may link to (use these exact hrefs, root-absolute e.g. /${links[0] || 'index.html'}): ${links.join(', ')}
Search ideas customers actually use (pick the ONE that makes the most useful, specific article for a real customer; prefer questions): ${candidates.join(' | ') || kws.join(' | ') || '(none — pick a common customer question this business answers)'}
Topics already published (do NOT repeat or closely overlap): ${st.posts.map((p) => p.title).join(' | ') || '(none yet)'}

TASK: write ONE blog post that genuinely helps someone searching for the chosen idea.
- 450-650 words, 3-5 H2 sections, short paragraphs, plain helpful tone in the business's own voice. Answer the question directly near the top. Do NOT include the H1 or any header/footer/nav — start with the first paragraph (the page adds the title and the date).
- Use ONLY facts stated on the homepage. NEVER invent prices, dollar amounts, percentages, statistics, guarantees, awards, testimonials, years in business, names or addresses. General, well-known, timeless advice is fine; if you are not sure a claim is true, leave it out. No medical, legal or financial promises.
- Link to 1-3 of the listed pages, and end with a short natural call to action pointing to the contact/booking page or phone number exactly as the homepage states.
- Allowed tags only: p, h2, h3, ul, ol, li, a, strong, em.

Reply in EXACTLY this format:
TOPIC: the search idea you chose, in a few words
SUMMARY: ONE plain-English sentence for the business owner, e.g. "Published a new blog post: How often should you detail your car?"
COMMIT: commit message
SLUG: url-slug-of-the-post
TITLE: post title (natural, under 60 chars)
DESCRIPTION: meta description under 155 chars
TEASER: one sentence teaser for the blog list
---BEGIN CONTENT---
the article html
---END CONTENT---`;
    const fr = await callAgent(key, planSystem, { prefix: fuser }, 4000);
    await spend(fr.usd);
    const g = (k) => (fr.text.match(new RegExp('^' + k + ':\\s*(.+)$', 'im')) || [])[1]?.trim() || '';
    const body = (fr.text.match(/---\s*BEGIN CONTENT\s*---\r?\n([\s\S]*?)\r?\n---\s*END CONTENT\s*---/i) || [])[1] || '';
    const topic = g('TOPIC');
    const title = g('TITLE').slice(0, 90);
    const desc = g('DESCRIPTION').slice(0, 170);
    const teaser = g('TEASER').slice(0, 220);
    const slug = g('SLUG').toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '').slice(0, 70);
    const path = `blog/${slug}.html`;
    const homeLower = plainText(template).toLowerCase();
    const inv = (plainText(body).toLowerCase().match(/\$\s?\d[\d,.]*|\b\d+(\.\d+)?\s?%/g) || []).filter((n) => !homeLower.includes(n.replace(/\s+/g, ' ')));
    const badTags = /<(script|style|iframe|form|img|h1|header|footer|nav)\b/i.test(body);
    if (!topic || !title || !desc || slug.length < 3 || tree.files.includes(path) || badTags || wordCount(body) < 380 || body.length > 30000 || inv.length || looksSpammy(body)) {
      return fail('blog post rejected: ' + (inv.length ? 'the draft stated numbers not found on the site (' + inv.slice(0, 2).join(', ') + ')' : !topic || !title || !desc ? 'reply was missing required parts' : 'the draft did not pass the checks'));
    }
    const at = (re) => (re.exec(indexHtml) || [])[0];
    const shell = (/<!-- POST-SHELL:([\s\S]*?)-->/.exec(indexHtml) || [])[1];
    const card = (/<!-- CARD-TEMPLATE:([\s\S]*?)-->/.exec(indexHtml) || [])[1];
    const start = indexHtml.indexOf('<!-- BLOG-MAIN-START -->') + '<!-- BLOG-MAIN-START -->'.length;
    const end = indexHtml.indexOf('<!-- BLOG-MAIN-END -->');
    // some sites' wrapper already carries its own <h1>{{TITLE}}</h1>: fill every placeholder,
    // and only add our own heading when the wrapper doesn't have one
    const filled = shell.replace(/\{\{TITLE\}\}/g, () => esc(title)).replace(/\{\{DATE\}\}/g, () => today).replace(/\{\{TEASER\}\}/g, () => esc(teaser));
    const head = /<h1[\s>]/i.test(filled) ? '' : `<h1>${esc(title)}</h1>\n`;
    const postMain = filled.replace('{{CONTENT}}', () => `${head}<p><time datetime="${today}">${today}</time></p>\n${body}`);
    const url = `https://${host}/${path}`;
    const ld = `<script type="application/ld+json">${JSON.stringify({ '@context': 'https://schema.org', '@type': 'Article', headline: title, datePublished: today, description: desc, mainEntityOfPage: url, author: { '@type': 'Organization', name: site.name }, publisher: { '@type': 'Organization', name: site.name } })}</script>`;
    let page = indexHtml.slice(0, start) + '\n' + postMain + '\n' + indexHtml.slice(end);
    page = page.replace(/<title>[\s\S]*?<\/title>/i, () => `<title>${esc(title)} | ${esc(site.name)}</title>`);
    page = /<meta[^>]+name=["']description["'][^>]*>/i.test(page) ? page.replace(/<meta[^>]+name=["']description["'][^>]*>/i, () => `<meta name="description" content="${esc(desc)}">`) : page.replace('</head>', () => `<meta name="description" content="${esc(desc)}">\n</head>`);
    page = /<link[^>]+rel=["']canonical["'][^>]*>/i.test(page) ? page.replace(/<link[^>]+rel=["']canonical["'][^>]*>/i, () => `<link rel="canonical" href="${url}">`) : page.replace('</head>', () => `<link rel="canonical" href="${url}">\n</head>`);
    page = page.replace('</head>', () => ld + '\n</head>');
    const cardHtml = card.replace(/\{\{URL\}\}/g, () => '/' + path).replace(/\{\{TITLE\}\}/g, () => esc(title)).replace(/\{\{TEASER\}\}/g, () => esc(teaser)).replace(/\{\{DATE\}\}/g, () => today);
    const newIndex = indexHtml.replace(BLOG_MARKER, () => cardHtml.trim() + '\n' + BLOG_MARKER);
    const outFiles = [{ path, content: page }, { path: 'blog/index.html', content: newIndex }];
    if (sitemap && sitemap.includes('</urlset>') && !sitemap.includes(url)) outFiles.push({ path: 'sitemap.xml', content: sitemap.replace('</urlset>', () => `<url><loc>${url}</loc></url>\n</urlset>`) });
    void at;
    const summary = g('SUMMARY') || `Published a new blog post: ${title}`;
    const res = await commitChangeset(site.repo, {
      files: outFiles,
      message: (g('COMMIT') || `Blog: ${title}`).slice(0, 100),
      branchPrefix: 'seo-blog',
      autoMerge: !!site.agentAutoMerge,
      body: `**${summary}**\n\nTopic: "${topic}"\n\n_Written and published by the Inspiring Websites SEO agent._`,
    });
    st.posts.push({ topic, title, path, at: Date.now(), prUrl: res.prUrl });
    st.lastAt = Date.now();
    st.retryAt = 0;
    st.lastError = '';
    await saveBlogState(site.slug, st);
    await log(site.slug, { action: site.agentAutoMerge ? 'shipped' : 'PR opened', detail: `blog post: ${title}`, prUrl: res.prUrl });
    await noteShipped(site.slug, `Published a new blog post: “${title}” (${url})`);
    await notifyOwner(`New blog post published on ${site.name}: "${title}" — ${url}`, { subject: `Blog post: ${site.name}` }).catch(() => {});
    return { ok: true, action: 'change', summary, files: outFiles.map((f) => f.path), pr: res, blogPost: { title, url } };
  }
  const user = `Business: ${site.name} — ${site.url}
Date: ${today}
Search ideas customers actually use (pick the ONE that makes the most useful, specific article for a real customer; prefer questions): ${candidates.join(' | ') || kws.join(' | ') || '(none — pick a common customer question this business answers)'}
Topics already published (do NOT repeat or closely overlap): ${st.posts.map((p) => p.title).join(' | ') || '(none yet)'}
${(rr?.results || []).slice(0, 4).map((r) => (r.topTitles?.length ? `What ranks today for "${r.keyword}": ${r.topTitles.slice(0, 3).map((t) => '"' + t.title + '"').join('; ')}` : '')).filter(Boolean).join('\n')}

The site's homepage (reuse its <head>, header/nav and footer so the post matches the site; assets must load from the blog/ folder — use root-absolute paths):
${template.slice(0, 40000)}

The current blog home page (blog/index.html):
${indexHtml.slice(0, 12000)}
${sitemap ? `\nCurrent sitemap.xml:\n${sitemap.slice(0, 4000)}` : ''}

TASK: write ONE blog post that genuinely helps someone searching for the chosen idea.
- 500-700 words, one H1, 3-5 H2 sections, short paragraphs, a plain helpful tone in the business's own voice. Answer the question directly near the top.
- Use ONLY facts stated on the homepage (services, location, hours, contact). NEVER invent prices, dollar amounts, percentages, statistics, guarantees, awards, testimonials, years in business, names or addresses. General, well-known, timeless advice is fine; if you are not sure a claim is true, leave it out. No medical, legal or financial promises.
- Link to the homepage and to 1-2 relevant existing pages (relative/root-absolute links that really exist). End with a short, natural call to action that points to the site's contact/booking page or phone number as stated on the homepage.
- Unique <title> (≤60 chars), <meta name="description"> (≤155 chars), <link rel="canonical" href="https://${host}/blog/POST-SLUG.html">, and Article JSON-LD (headline, datePublished ${today}, author/publisher = the business).
- Show the date ${today} near the top of the article.
- If the homepage's styling is a large inline <style> block that cannot be reused, reply ONLY: NOOP: template uses large inline styles
- ALSO add a card for the new post to blog/index.html with ONE PATCH: FIND is exactly ${BLOG_MARKER} ; REPLACE is a card (title link, one-sentence teaser, date) copying the site's own styling, followed by ${BLOG_MARKER} again so the next post can be added.${sitemapRule}
...
---END PATCH---`;
  ...
  const plan = await finalizePlan(first.text, { repo: site.repo, branch: tree.branch });
  if (!plan.files.length && plan.noop) return fail('blog skipped — ' + plan.noop, { retryHours: 24 * 400, unsupported: plan.noop });
  const post = plan.files.find((f) => /^blog\/[a-z0-9][a-z0-9-]{2,80}\.html$/.test(f.path) && f.path !== 'blog/index.html');
  const topic = (first.text.match(/^TOPIC:\s*(.+)$/im) || [])[1]?.trim() || '';
  const homeText = plainText(template).toLowerCase();
  const bodyText = post ? plainText(post.content).toLowerCase() : '';
  const inventedNumbers = (bodyText.match(/\$\s?\d[\d,.]*|\b\d+(\.\d+)?\s?%/g) || []).filter((n) => !homeText.includes(n.toLowerCase().replace(/\s+/g, ' ')));
  const bad =
    !post ||
    !topic ||
    tree.files.includes(post.path) ||
    !/<title>[^<]{5,}/i.test(post.content) ||
    !/<h1[\s>]/i.test(post.content) ||
    !new RegExp(`rel=["']canonical["'][^>]*/blog/${post.path.slice(5).replace('.', '\\.')}`, 'i').test(post.content) ||
    wordCount(post.content) < 380 ||
    post.content.length > 60000 ||
    inventedNumbers.length > 0 ||
    looksSpammy(post.content);
  if (bad) {
    const why = !post ? 'no post file in the reply' : inventedNumbers.length ? 'the draft stated numbers not found on the site (' + inventedNumbers.slice(0, 2).join(', ') + ')' : 'the draft did not pass the checks' + (plan.patchErrors?.length ? ' (' + plan.patchErrors[0] + ')' : '');
    return fail('blog post rejected: ' + why);
  }
  const okFiles = plan.files.filter((f) => f.path === post.path || f.path === 'sitemap.xml' || (f.path === 'blog/index.html' && f.patched && f.content.includes(BLOG_MARKER) && f.content.includes(post.path.slice(5))));
  const title = (post.content.match(/<title>([^<]+)<\/title>/i) || [])[1]?.replace(/\s*[|\-–—].*$/, '').trim() || topic;
  const res = await commitChangeset(site.repo, {
    files: okFiles.map((f) => ({ path: f.path, content: f.content })),
    message: (plan.commitMessage || `Blog: ${title}`).slice(0, 100),
    branchPrefix: 'seo-blog',
    autoMerge: !!site.agentAutoMerge,
    body: `**${plan.summary || 'New blog post'}**\n\nTopic: "${topic}"\n\n_Written and published by the Inspiring Websites SEO agent._`,
  });
  const url = `https://${host}/${post.path}`;
  st.posts.push({ topic, title, path: post.path, at: Date.now(), prUrl: res.prUrl });
  st.lastAt = Date.now();
  st.retryAt = 0;
  await saveBlogState(site.slug, st);
  await log(site.slug, { action: site.agentAutoMerge ? 'shipped' : 'PR opened', detail: `blog post: ${title}`, prUrl: res.prUrl });
  await noteShipped(site.slug, `Published a new blog post: “${title}” (${url})`);
  await notifyOwner(`New blog post published on ${site.name}: "${title}" — ${url}`, { subject: `Blog post: ${site.name}` }).catch(() => {});
  return { ok: true, action: 'change', summary: plan.summary || `Published a new blog post: ${title}`, files: okFiles.map((f) => f.path), pr: res, blogPost: { title, url } };
}

function buildPlaybook({ keywords, rankLine, intel, files, htmlFiles, done, homepage, recover = [], newPageOk = false, blogDue = false }) {
  const M = MONTH();
  ...
  if (blogDue) items.push({ key: 'blog', kind: 'blog', pages: [], title: 'Publish this week’s blog post', detail: '' });
...
async function freeCycle(slug) {
  await store.set(`agent:lastCycleAt:${slug}`, String(Date.now() - ATTEMPT_GAP_MS + 5 * 60000), { ex: 60 * 60 * 24 * 45 }).catch(() => {});
}
...
export async function runAgentCycle(site, opts = {}) {
  ...
  // a manual "start the blog now" may ignore the once-a-day spacing (the blog is a weekly
  // extra), but never the budget cap, failure pause, or a blocked repo
  if (opts.blogNow) reasons = reasons.filter((r) => !/^paced — (a change just shipped|a cycle just ran)/.test(r));
...
async function runAgentCycleInner(site, st, { manual = false, ctx = {}, blogNow = false } = {}) {
  const key = process.env.ANTHROPIC_API_KEY_AGENT || process.env.ANTHROPIC_API_KEY;
  const m = MONTH();
  ...
  if (!targetItem) {
    ...
    const pbList = buildPlaybook({
      ...
      blogDue: await blogDue(site, blogNow),
      files: tree.files,
      htmlFiles,
      done,
      homepage,
    }).filter((i) => !stuck.has('pb:' + i.key));
    // Order of importance: (1) winning back a keyword that just dropped, (2) the
    // foundation Google needs to find and understand the site at all (sitemap,
    // robots.txt, llms.txt, business structured data), and only THEN (3) the
    // open to-dos / audit findings, and the rest of the monthly rotation. It used
    // to take to-dos first, so a site with no sitemap at all got page-speed hints
    // shipped ahead of the one file that lets Google discover its pages.
    const FOUNDATION = new Set(['sitemap', 'robots', 'llms', 'schema:home']);
    // the weekly blog post sits right behind the foundation: it is due at most once
    // a week, so it never crowds out the daily work, but it also can't be starved by it
    const urgent = (blogNow && pbList.find((i) => i.kind === 'blog')) || pbList.find((i) => i.recover || FOUNDATION.has(i.key) || i.kind === 'blog');
    const pb = urgent || (pool.length ? null : pbList[0]);
    if (pb) {
  ...
  if (playbookKind === 'blog') {
    try {
      const home = htmlFiles.find((p) => /(^|\/)index\.html?$/i.test(p)) || htmlFiles[0];
      const r = await runBlog(site, { key, spend, tree, homepageFile: home });
      if (r.action === 'change') await store.set(`agent:lastShipAt:${site.slug}`, String(Date.now()), { ex: 60 * 60 * 24 * 45 }).catch(() => {});
      if (r.action === 'blog-setup') await freeCycle(site.slug); // the first post follows straight away
      if (r.ok === false) await noteFailure(site.slug, r.error);
      return { slug: site.slug, ranks, status: await agentStatus(site), ...r };
    } catch (e) {
      await noteFailure(site.slug, e.message || e);
      return { ok: false, slug: site.slug, error: 'blog failed: ' + (e.message || e), ranks };
    }
  }
  if (playbookKind === 'newpage') {
    try {
      ...
      if (r.action === 'new-page-proposed') await store.set(`agent:lastShipAt:${site.slug}`, String(Date.now()), { ex: 60 * 60 * 24 * 45 }).catch(() => {});
  ...
  // Sites often keep the exact markup for a repeated block in a template/notes file
  // (e.g. sessions.html.example with the session-card markup, incl. the <br> that keeps
  // price and button apart). A revision that adds such a block must copy it, so show it.
  if (pendingRevision) {
    for (const p of tree.files.filter((f) => /\.(example|template|tpl|sample)$/i.test(f)).slice(0, 3)) {
      if (budgetLeft <= 500) break;
      const c = await getFileContent(site.repo, p, tree.branch);
      if (!c) continue;
      const take = c.slice(0, Math.min(15000, budgetLeft));
      sample[p] = take;
      budgetLeft -= take.length;
    }
  }
  ...
  const revisionAttachment = pendingRevision && pendingRevision.ticketId ? String((await store.get(`revisions:attach:${pendingRevision.ticketId}`).catch(() => '')) || '').slice(0, 30000) : '';
...
${targetItem.detail || ''}
${pendingRevision ? `\nCONTENT REQUESTS — rules when the client gives you content (events, services, prices, team, hours…):\n- COMPLETE: every item they list must appear, and each item keeps its OWN description, details, dates, times, price, audience and link exactly as written (fix only obvious typos). Never shorten a description to a one-line teaser, never merge two items, never drop a paragraph or bullet list.\n- ORDER: dates in chronological order unless they asked otherwise.\n- PLACE: put it where the client said ("events tab" = the site's events/sessions area). Do not put it in an unrelated section and do not rewrite copy that is not part of the request.\n- REUSE THE SITE'S OWN MARKUP: if a template/example file or an existing block of the same kind is shown, copy its structure and classes exactly (including line breaks between a price and a button) — do not invent new markup that could look different.\n- Old placeholder text that contradicts the new content (e.g. "dates coming soon") must be removed or updated everywhere it appears, including any chat/FAQ text.\n` : ''}${revisionAttachment ? `\nTHE CLIENT'S FULL EMAIL AND THE TEXT OF ANY FILE THEY ATTACHED (this is the source of truth for the content to put on the site — use the dates, prices, names and links exactly as written, and do not invent anything it does not contain):\n${revisionAttachment}\n` : ''}
...
---END CONTENT---`;
  ...
  if (!safe) {
    // say WHICH rule tripped — "rejected by safety check" alone hid a false positive for days
    const why = files.length === 0 ? 'no files' : files.length > 6 ? 'more than 6 files' : files
      .map((f) =>
        f.path.includes('..') ? `${f.path}: bad path` :
        !knownOrNew(f.path) ? `${f.path}: not one of the files shown to the model` :
        visualBlockApplies(f.path) ? `${f.path}: image/stylesheet edits are not allowed for automatic work` :
        f.patched ? (looksSpammy(f.replaceText) ? `${f.path}: added text looks like keyword stuffing` : f.replaceText.length >= 20000 ? `${f.path}: change too large` : '') :
        looksSpammy(f.content) ? `${f.path}: page looks like keyword stuffing` : f.content.length >= 60000 ? `${f.path}: file too large` : ''
      )
      .filter(Boolean)
      .join('; ');
    await log(site.slug, { action: 'skipped', detail: `Plan rejected by safety check (${why || 'unknown'}): ${plan.summary || ''}` });
    let reason = 'plan failed safety check — ' + (why || 'unknown');
    if (pendingRevision) {
      const n = await bumpRevisionFailure(pendingRevision.id);
  ...
  try {
    ...
    await log(site.slug, {
      ...
      files: files.map((f) => f.path).slice(0, 8),
      prUrl: res.prUrl,
      branch: res.branch,
    });
    if (pendingRevision) await clearRevisionFailure(pendingRevision.id);
    else {
      await store.set(`agent:lastShipAt:${site.slug}`, String(Date.now()), { ex: 60 * 60 * 24 * 45 }).catch(() => {});
