...
import { summarizeRanks, appendRankHistory, saveRanks } from './ranks.js';
...
const UNSHIPPABLE = /(compress\w*|resiz\w*|shrink\w*|optimi[sz]\w*|convert\w*|reduc\w*|serv\w*|replac\w*)\b[^.\n]{0,50}\b(images?|photos?|hero|videos?|fonts?|files?)\b|\b(webp|avif)\b|\blargest content\w*|\blcp\b|\bcore web vitals\b/i;
...
function buildPlaybook({ keywords, rankLine, files, htmlFiles, done, homepage }) {
  const M = MONTH();
  ...
  if (!has(/(^|\/)sitemap\.xml$/i)) {
    items.push({ key: 'sitemap', title: 'Add a sitemap.xml so Google finds every page', detail: `Create sitemap.xml at the site root listing every page (${pages.join(', ')}) using the live domain URLs.` });
  }
  if (!has(/(^|\/)robots\.txt$/i)) {
    items.push({ key: 'robots', title: 'Add a robots.txt that points Google to the sitemap', detail: 'Create robots.txt allowing all crawlers and referencing the sitemap URL.' });
  }
  if (!has(/(^|\/)llms\.txt$/i)) {
    items.push({ key: 'llms', title: 'Add an llms.txt so AI search tools describe the business correctly', detail: 'Create llms.txt: business name, what it does, where it serves, and the key pages — only facts stated on the site.' });
  }
