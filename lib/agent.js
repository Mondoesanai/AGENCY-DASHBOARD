...
export async function runAgentCycle(site, opts = {}) {
  ...
  // doesn't wait on the monthly SEO exploration budget or the pacing floor
  // that's meant to spread THAT spend across the month. The 3-strike
  // circuit breaker (bumpRevisionFailure) is what actually protects against
  // runaway cost on a revision that keeps failing, not this cap.
  ...
  const reasons = hasPendingRevision ? st.reasons.filter((r) => !r.includes('budget is used') && !r.startsWith('paced')) : st.reasons;
