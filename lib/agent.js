...
function parsePlan(text) {
  ...
  const commitMessage = (s.match(/^COMMIT:\s*(.+)$/m) || [])[1]?.trim() || '';
  const files = [];
  const re = /^FILE:\s*(.+?)\s*\r?\nREASON:\s*(.*?)\s*\r?\n---BEGIN CONTENT---\r?\n([\s\S]*?)\r?\n---END CONTENT---/gm;
  let m;
...
async function runAgentCycleInner(site, st, { manual = false } = {}) {
  ...
  // the SAME findings Mondo sees on the site's To-dos tab — the agent works
  // straight off that visible checklist, so a finding here is a finding fixed
  // there (once the next audit refresh confirms it, buildFindings() just stops
  // surfacing it — no separate "done" bookkeeping needed).
  const findings = buildFindings(audit, null)
    .filter((f) => f.severity !== 'good')
    .slice(0, 8);

  let plan;
  await setRunning(site.slug, 'Planning the next technical-SEO fix');
  try {
    const { text, usd } = await callAgent(
      key,
      'You are a senior technical-SEO engineer editing a real client website. Make ONE focused, safe improvement. Never keyword-stuff. Reply in the exact plain-text format requested — no JSON, no markdown code fences.',
      `Site: ${site.name} — ${site.url}
Target keywords: ${keywords.join(', ')}
Repo files (${tree.files.length}): ${tree.files.slice(0, 120).join(', ')}
Existing file contents:
${Object.entries(sample).map(([p, c]) => `--- ${p} ---\n${c}`).join('\n\n') || '(none fetched)'}
Open builder to-dos, highest priority first: ${findings.length ? findings.map((f) => `[${f.severity}] ${f.title} — ${f.detail}`).join(' | ') : 'none open'}

Pick the single highest-impact item from that to-do list that fits what you can see, and fix it. Examples: add/repair <title> + meta description on a page, add LocalBusiness JSON-LD, create/improve llms.txt, add a sitemap.xml or robots.txt, add descriptive alt text, add internal links to key pages, fix a heading hierarchy, tighten body copy. Only edit files you were shown the full content of, or CREATE new small files (llms.txt, robots.txt, sitemap.xml).
...
---END CONTENT---`,
    ...
    await spend(usd);
    plan = parsePlan(text);
    if (!plan.files.length) throw new Error('model reply did not match the expected FILE/CONTENT format');
  } catch (e) {
