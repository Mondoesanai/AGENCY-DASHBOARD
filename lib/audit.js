...
export async function runAudit(url, { fresh = false, cachedOnly = false } = {}) {
  const cacheKey = `audit:${url}`;
  if (!fresh) {
    const cached = await store.get(cacheKey);
    if (cached) return typeof cached === 'string' ? JSON.parse(cached) : cached;
  }
  // dashboard feed asks for cache-only so it never blocks on a cold PageSpeed call;
  // the browser then warms each missing audit in the background.
  if (cachedOnly) return { url, ok: false, pending: true, error: 'audit not run yet' };
