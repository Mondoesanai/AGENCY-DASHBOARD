// Generates the monthly report card as an SVG. Dark, Inspiring Websites branded.
// Used as the dashboard/report thumbnail and linked from the client email.
// Pure string building — no browser, no native deps.
...
function chart(x, y, w, h, history) {
  ...
  if (rows.length === 1) {
    const v = rows[0].visitors || 0;
    return `<g transform="translate(${x},${y})">${head}
      <line x1="40" y1="${h - 40}" x2="${w - 40}" y2="${h - 40}" stroke="${C.line}"/>
      <circle cx="${w - 46}" cy="${h - 40}" r="6" fill="${C.pos}" stroke="${C.panel2}" stroke-width="2"/>
      <text x="${w - 46}" y="${h - 54}" fill="${C.pos}" font-size="16" font-weight="700" text-anchor="end">${nf(v)}</text>
      <text x="40" y="${h - 16}" fill="${C.faint}" font-size="13">Month 1 — the trend line grows from here</text>
    </g>`;
  }
  if (rows.length < 1) {
    return `<g transform="translate(${x},${y})">${head}
      <text x="${w / 2}" y="${h / 2 + 8}" fill="${C.faint}" font-size="17" text-anchor="middle">Visitor trend appears once tracking has run a while</text>
    </g>`;
  }
...
// The report card is served and embedded as inline SVG everywhere (drawer, /r/
// page, /api/card). We used to rasterize it to PNG with `sharp` for the email
// attachment, but sharp's native install is a recurring source of serverless
// build failures, so it's gone. Callers already fall back to SVG / no attachment
