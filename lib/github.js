...
async function gh(path, opts = {}) {
  // a slow or dropped GitHub call (timeout / 5xx) used to fail the whole cycle — three times
  // in a row on one site. Read calls and blob/tree/commit creation are safe to repeat, so retry those once.
  const safe = !opts.method || opts.method === 'GET' || /\/git\/(blobs|trees|commits)$/.test(path);
  let r;
  for (let attempt = 0; ; attempt++) {
    try {
      r = await fetch(GH + path, { headers: headers(), ...opts, signal: AbortSignal.timeout(opts.timeout || 25000) });
      if (safe && attempt === 0 && r.status >= 500) continue;
      break;
    } catch (e) {
...
// "Publish it?" — merge a pull request that was opened WITHOUT auto-merge
// (used for changes big enough to want a human yes, e.g. a whole new page).
export function prNumberFromUrl(url) {
  const m = String(url || '').match(/\/pull\/(\d+)/);
  return m ? Number(m[1]) : null;
}
export async function mergePullRequest(repo, number) {
  await gh(`/repos/${repo}/pulls/${number}/merge`, { method: 'PUT', body: JSON.stringify({ merge_method: 'squash' }) });
  return { ok: true };
}
export async function closePullRequest(repo, number) {
