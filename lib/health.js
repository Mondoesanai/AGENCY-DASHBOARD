...
export async function systemHealth() {
  ...
  else if (now - alive > 12 * 3600000) issues.push({ level: 'critical', text: `The SEO automation hasn't checked in for ${Math.round((now - alive) / 3600000)} hours — last seen ${new Date(alive).toLocaleString()}. Check the Actions tab on the GitHub repo.` });
  else if (now - alive > 3 * 3600000) issues.push({ level: 'warn', text: `The SEO automation last checked in ${Math.round((now - alive) / 3600000)} hours ago (it should be every ~30 minutes).` });
