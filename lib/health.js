...
export async function systemHealth() {
  ...
  budgets.forEach((b) => {
    if (b.exhausted) issues.push({ level: 'warn', text: `${b.name}'s SEO budget is used up ($${b.spent}/$${b.cap}) — it'll stop shipping automatically until next month (or raise the cap in Settings).` });
  });
