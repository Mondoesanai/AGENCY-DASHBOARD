...
export async function systemHealth() {
  ...
  // the things that decide whether a site can rank at all
  {
    const freeHosts = [];
    for (const s of sites) {
      ...
      let r = null;
      try {
        const raw = await store.get(`agent:ranks:${s.slug}`);
        r = raw ? (typeof raw === 'string' ? JSON.parse(raw) : raw) : null;
      } catch {
        r = null;
      }
      if (r && r.indexed && r.indexed.count === 0) {
        issues.push({ level: 'warn', text: `${s.name}: Google shows none of this site's pages as indexed. A page Google hasn't indexed cannot rank, so this is the first thing to fix — verify the site in Google Search Console and submit its sitemap (only the site owner can do that step).` });
      }
      if (/\.vercel\.app(\/|$)/i.test(s.url || '')) freeHosts.push(s.name);
    }
    if (freeHosts.length) {
      issues.push({ level: 'info', text: `${freeHosts.join(' and ')} ${freeHosts.length === 1 ? 'runs' : 'run'} on a free vercel.app address. Google will index it, but a custom domain of the client's own ranks noticeably better and looks more trustworthy.` });
    }
  }
  if (!env.sms) issues.push({ level: 'info', text: 'Text messaging is not set up yet (needs TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, TWILIO_FROM and OWNER_PHONE in Vercel) — until then every alert and question comes by email instead, and new-page approvals are switched off.' });
  ...
  // the real heartbeat is the GitHub-driven auto-tick (every ~30 min); the
  // Vercel cron marker is only a bonus signal for the once-a-day pass.
  const tickLast = await num('auto:lastTick');
  const alive = Math.max(cronLastRun, tickLast);
  if (!alive) issues.push({ level: 'critical', text: 'The SEO automation has never checked in — the GitHub Actions schedule (.github/workflows/seo-automation.yml) needs its CRON_SECRET repo secret and Actions enabled.' });
  // GitHub's free scheduler is bursty (a "10 minute" job really ran every 4-6h), so
  // normal gaps are hours — only flag when it's genuinely stalled.
  else if (now - alive > 24 * 3600000) issues.push({ level: 'critical', text: `The SEO automation hasn't checked in for ${Math.round((now - alive) / 3600000)} hours — last seen ${new Date(alive).toLocaleString()}. Check the Actions tab on the GitHub repo.` });
  else if (now - alive > 9 * 3600000) issues.push({ level: 'warn', text: `The SEO automation last checked in ${Math.round((now - alive) / 3600000)} hours ago — GitHub's scheduler is running slow. Opening the dashboard also triggers it.` });
  ...
  if (untagged.length) {
    issues.push({ level: 'info', text: `${untagged.length} site${untagged.length === 1 ? '' : 's'} haven't had conversion tracking set up yet — it happens automatically on their next automation pass, nothing to press.` });
  }
