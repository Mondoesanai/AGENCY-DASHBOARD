...
export function summarizeRanks(ranksObj) {
  ...
  // When nothing's showing up in the search depth yet (common for a brand
  // new site — could be months), avgRank is null and there was previously
  // nothing to show but a blank dash. Google's own "about N results" count
  // for the target keywords is at least SOMETHING that visibly shrinks as
  // the site starts actually competing — not a real rank, so it's kept
  // separate and the UI has to label it honestly, but it beats a dash.
  // DataForSEO's live endpoint doesn't reliably return this field on every
  // call (some checks come back as flat errors, others just omit it) — a
  // real case showed only 1 of 12 keywords with a usable count, which
  // produced a wildly unrepresentative single-sample "average." Require a
  // real sample before trusting it enough to show.
  const withCount = rows.filter((r) => r.resultsCount);
  const roughField = withCount.length >= 4
    ? Math.round(withCount.reduce((t, r) => t + r.resultsCount, 0) / withCount.length)
    : null;
  return {
    ...
    inTop3,
    inTop10,
    roughField: avgRank == null ? roughField : null,
    indexed: ranksObj.indexed || null,
...
// Saves a fresh rank snapshot, first stashing the previous one (if it's at
// least half a day older) so reports can say "moved from #34 to #22" per
// keyword instead of only ever showing the latest number.
export async function saveRanks(slug, ranks) {
  ...
  try {
    ...
    if (cur && cur.at && (ranks.at || Date.now()) - cur.at > 12 * 3600000) {
      await store.set(`agent:ranksPrev:${slug}`, JSON.stringify(cur), { ex: 60 * 60 * 24 * 120 }).catch(() => {});
    }
  } catch {
    /* no usable previous snapshot */
  }
  await store.set(`agent:ranks:${slug}`, JSON.stringify(ranks));
}
export async function readPrevRanks(slug) {
  const raw = await store.get(`agent:ranksPrev:${slug}`).catch(() => null);
  try {
...
// per-keyword table for reports: current position, previous position, change
export function keywordTable(cur, prev) {
  if (!cur || !Array.isArray(cur.results)) return [];
  const before = new Map((prev?.results || []).filter((r) => !r.error).map((r) => [r.keyword, r.rank || null]));
  return cur.results
    .filter((r) => !r.error)
    .map((r) => {
      const p = before.has(r.keyword) ? before.get(r.keyword) : undefined;
      let change = null; // + = improved (moved up), - = dropped
      if (p !== undefined) {
        if (r.rank && p) change = p - r.rank;
        else if (r.rank && !p) change = 999; // newly ranking
        else if (!r.rank && p) change = -999; // fell out
        else change = 0;
      }
      return { keyword: r.keyword, rank: r.rank || null, prevRank: p === undefined ? null : p, change };
    })
