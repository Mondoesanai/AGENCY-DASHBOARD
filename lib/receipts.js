...
import { allAiCost } from './aicost.js';
...
async function buildLedger() {
  ...
  // tracked AI / automation spend — one record per month
  try {
    const ai = await allAiCost();
    for (const r of ai.rows) {
      if (r.total <= 0) continue;
      expenses.push({
        id: `EXP-AI-${r.month.replace('-', '')}`,
        type: 'expense',
        date: `${r.month}-28`,
        time: '—',
        counterparty: 'Anthropic / automation',
        description: `AI & automation — Compass $${r.coach.toFixed(2)} + SEO agent $${r.agent.toFixed(2)} (${r.month})`,
...
export async function receiptsHandler(req, res) {
