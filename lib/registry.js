...
// Loosest possible comparison key — strips the TLD/preview-hosting suffix
// (via hostKey) AND every non-alphanumeric character, so "setapartmovement.com"
// and "set-apart-movement.vercel.app" (the same project on its custom domain
// vs. its raw Vercel URL — two genuinely different hostnames a browser can't
// tell apart) both collapse to "setapartmovement" and are recognized as one
// site instead of silently registering a duplicate.
function coreProjectKey(input) {
  let s = hostKey(input);
  s = s.replace(/\.(vercel\.app|netlify\.app|com|net|org|co|io|us|biz|app|dev)$/i, '');
  return s.replace(/[^a-z0-9]+/g, '');
}

// Find a site (any source) whose URL's core project name matches this new
// host, so a beacon or a manual "Add site" from an alternate hostname of a
// site you already track lands on the SAME slug instead of a fresh duplicate.
export async function matchExistingSite(input) {
  const key = coreProjectKey(input);
  if (!key) return null;
  const sites = await listSites().catch(() => []);
  const hit = sites.find((s) => s.url && coreProjectKey(s.url) === key);
...
export async function saveSiteConfig(slug, patch) {
  ...
  if (patch.repo !== undefined)
    merged.repo = String(patch.repo || '').trim().replace(/^https?:\/\/github\.com\//i, '').replace(/\.git$/, '').slice(0, 120);
  if (patch.seoAgent !== undefined) merged.seoAgent = !!patch.seoAgent;
  if (patch.agentAutoMerge !== undefined) merged.agentAutoMerge = !!patch.agentAutoMerge;
  if (patch.agentKeywords !== undefined)
    merged.agentKeywords = String(patch.agentKeywords || '').split(/[,\n]/).map((s) => s.trim()).filter(Boolean).slice(0, 15).join(', ');
  if (patch.revisionsAuto !== undefined) merged.revisionsAuto = !!patch.revisionsAuto;
  if (patch.agentBudget !== undefined)
    merged.agentBudget = Math.min(20, Math.max(0, Number(String(patch.agentBudget).replace(/[^0-9.]/g, '')) || 18));
  if (patch.agentCap !== undefined)
    merged.agentCap = Math.min(20, Math.max(0, Number(String(patch.agentCap).replace(/[^0-9.]/g, '')) || 20));
  if (patch.blog !== undefined) merged.blog = !!patch.blog;
  if (patch.reportEvery !== undefined) merged.reportEvery = patch.reportEvery === 'biweekly' ? 'biweekly' : 'monthly';
  ...
  extra.forEach((slug, i) => {
    ...
    if (cfg) {
      bySlug.set(slug, {
        ...
        reportEvery: cfg.reportEvery === 'biweekly' ? 'biweekly' : 'monthly',
        leadSource: cfg.leadSource || '',
        leadSourceDate: cfg.leadSourceDate || 0,
        repo: cfg.repo || '',
        seoAgent: cfg.seoAgent !== false,
        blog: cfg.blog !== false,
        // always on, no per-site toggle — every site auto-merges once a fix
        // clears the safety check. (agentAutoMerge is still accepted on save
        // for a future manual override; nothing in the UI sends it anymore.)
        agentAutoMerge: true,
        agentKeywords: cfg.agentKeywords || '',
        revisionsAuto: cfg.revisionsAuto !== false,
        agentBudget: cfg.agentBudget || 18,
        agentCap: cfg.agentCap || 20,
