...
import { listNewMail, getMessageById, unlabelProcessed, threadHasSentMessage, labelProcessed, createCalendarEvent, updateCalendarEvent, deleteCalendarEvent, sendGmailReply, sendGmailMessage, getMyEmailAddress, googleConfigured } from './google.js';
...
import { askOwner, notifyOwner } from './sms.js';
...
// A "Website presentation meeting report discussing redesign..." from
// executiveassistant@e.read.ai got auto-replied to and calendar-held as a
// real revision request — a meeting-notes bot's summary, not a client
// asking for anything. The model saw website-related keywords and read too
// much into them. Known automated senders never send a specific, personal
// change request, so they're excluded before the classifier ever sees
// them — cheaper, and removes any chance of content fooling it.
const AUTOMATED_SENDER_RE = /^(no-?reply|do-?not-?reply|notifications?|mailer-?daemon|automated|alerts?|bot|calendar|scheduling)@/i;
const AUTOMATED_DOMAIN_RE = /(^|\.)(e\.read\.ai|read\.ai|otter\.ai|fireflies\.ai|grain\.co|fathom\.video|gong\.io|chorus\.ai|avoma\.com|tldv\.io|calendly\.com|docusign\.net|zoom\.us)$/i;
function looksAutomated(fromHeader) {
  const email = emailOf(fromHeader);
  return AUTOMATED_SENDER_RE.test(email) || AUTOMATED_DOMAIN_RE.test(domainOf(email));
}

// The bot was replying to ANYONE the classifier read as asking for a change —
// a contact from a meeting, someone answering an email Mondo sent ("thanks for
// sending the site!") — with "we'll have it updated in 3 business days," and
// even did so when it couldn't match the email to any client site at all.
// A reply is only ever sent to a verified client sender: the address on file
// for a site, someone Mondo already confirmed (assign/correct), or an address
// on that client's own domain. Everyone else gets no reply, ever — at most a
// held ticket on the dashboard for Mondo to look at.
const FREE_MAIL_RE = /(^|\.)(gmail|googlemail|yahoo|ymail|outlook|hotmail|live|msn|icloud|me|aol|proton|protonmail|gmx|comcast|att|verizon)\.[a-z.]+$/i;
const hostOf = (u) => String(u || '').replace(/^https?:\/\//i, '').replace(/^www\./i, '').replace(/[/?#].*$/, '').toLowerCase();
// Mondo himself (the agency owner) re-sending or forwarding a client's request from his
// personal address is an instruction, not a stranger — he said so explicitly.
const ownerEmails = () => [...new Set([...String(process.env.OWNER_EMAIL || '').split(/[,;\s]+/), 'mondoesanai@gmail.com', 'media.mondoe@gmail.com'].map((x) => x.trim().toLowerCase()).filter(Boolean))];
async function senderTrust(fromHeader, sites) {
  const email = emailOf(fromHeader);
  const dom = domainOf(email);
  if (ownerEmails().includes(email)) return { trusted: true, owner: true, slug: null, why: 'the agency owner' };
  for (const s of sites) {
    const onFile = String(s.email || '').toLowerCase().split(/[,;\s]+/).filter(Boolean);
    if (onFile.includes(email)) return { trusted: true, slug: s.slug, why: 'address on file' };
  }
  const known = await knownSlugFor(email);
  if (known && sites.some((s) => s.slug === known)) return { trusted: true, slug: known, why: 'previously confirmed' };
  if (dom && !FREE_MAIL_RE.test(dom)) {
    const own = sites.find((s) => {
      const h = hostOf(s.url);
      return h && (dom === h || dom.endsWith('.' + h) || h.endsWith('.' + dom));
    });
    if (own) return { trusted: true, slug: own.slug, why: "client's own domain" };
  }
  return { trusted: false, slug: null, why: 'not a known client address' };
}
...
async function classify(mail, sites, trust) {
...
${mail.attachText ? `\nThe email has attached file(s); their text is below. Treat it as part of what the sender is asking for (for example a list of events/prices/hours to put on the site):\n${mail.attachText.slice(0, 6000)}\n` : ''}${mail.attachNotes && mail.attachNotes.length ? `\n(Attachments that could not be read: ${mail.attachNotes.join('; ')})\n` : ''}
Sender verification (checked in code, not by you): ${trust && trust.owner ? 'this sender is the AGENCY OWNER, who is passing along a client\'s request from his personal address. Whatever change he describes or attaches IS a real instruction; pick the site from the business/person named in the email or the attached file (slug null only if there is truly no clue).' : trust && trust.trusted ? `this sender IS a verified client of ${trust.slug} (${trust.why}).` : `this sender is NOT a verified client — treat them as an unknown person: a meeting contact, friend, prospect, vendor, or someone replying to an email the agency owner sent.`}

Is this an existing website client giving a direct instruction to change/update THEIR website (content edit, add/remove something, fix something visible)? Ignore newsletters, spam, invoices, sales pitches, and anything not about an existing client site's content. A question is NOT a request. Polite chatter, thanks, acknowledgements, scheduling, replies to something the agency owner wrote ("thanks for sending the site", "sounds good", "got it"), and someone asking for or discussing a NEW website are all isRevision:false. If the sender is NOT a verified client, isRevision must be false unless the email explicitly names one of the client sites/businesses above AND gives a specific change instruction.

CRITICAL — a request vs. a report about one are NOT the same thing. Only mark isRevision:true for a direct, specific instruction from the client themselves ("please change X to Y", "can you add/remove Z", "the phone number is wrong, it should be..."). Mark isRevision:false for: a meeting summary, notes, or report that merely DISCUSSES the website (even one listing specific issues like "weak navigation" or "unclear CTAs" — that's an analysis, not a request); a third-party tool's automated output (meeting-notes bots, transcripts, scheduling confirmations); anything written in the third person or as an observation rather than a direct ask. When genuinely unsure whether it's a real request or just talk about the site, set confident:false rather than guessing isRevision:true.
...
JSON: {"isRevision": true|false, "slug": "<matching site slug from the list, or null if you can't tell>", "confident": true|false, "summary": "one short specific line describing exactly what they're asking for, empty string if not a revision"}`,
  ...
  try {
    ...
    return { isRevision: !!parsed.isRevision, slug: parsed.slug || null, confident: parsed.confident === true, summary: String(parsed.summary || '').slice(0, 200) };
  } catch (e) {
...
async function sendRevisionCompletedEmail(site, ticket) {
  ...
  const body =
    ...
    `• ${ticket.shipSummary || ticket.summary}\n\n` +
...
async function verifyShippedFix(site, ticket) {
  ...
  // check the page(s) that actually changed (the request is rarely about the homepage)
  const base = String(site.url || '').replace(/\/+$/, '');
  const paths = (ticket.shipFiles || []).filter((p) => /\.html?$/i.test(p)).slice(0, 3);
  const urls = paths.length ? paths.map((p) => (/(^|\/)index\.html?$/i.test(p) ? base + '/' : base + '/' + p.replace(/^\/+/, ''))) : [base];
  // keep link targets: buttons carry the requested payment/registration URLs, which plain text loses
  const strip = (h) => h.replace(/<a\b[^>]*?href=["']([^"']+)["'][^>]*>/gi, ' [link: $1] ').replace(/<script[\s\S]*?<\/script>/gi, ' ').replace(/<style[\s\S]*?<\/style>/gi, ' ').replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim();
  let text = '';
  try {
    for (const u of urls) {
      const r = await fetch(u, { redirect: 'follow', signal: AbortSignal.timeout(10000), headers: { 'Cache-Control': 'no-cache' } });
      text += `\n[${u}]\n` + strip(await r.text()).slice(0, 9000);
    }
  } catch (e) {
    return { ok: true, verified: null, note: 'skipped — could not fetch the live site' };
  }
  const request = String((await store.get(`revisions:attach:${ticket.id}`).catch(() => '')) || ticket.summary || '').slice(0, 5000) || ticket.summary;
  let Anthropic;
  try {
    ({ default: Anthropic } = await import('@anthropic-ai/sdk'));
  } catch (e) {
  ...
  try {
    r = await client.messages.create({
      ...
      messages: [
        {
          role: 'user',
          content: `Client asked for: ${ticket.summary}\n\nTheir full request:\n${request}\n\nLive page text of the page(s) that were changed:${text}\n\nCheck COMPLETENESS, not just presence: is every item they listed on the page, with its own description/details/dates/links, and are dates in order? matches=false ONLY if something clearly requested is missing or wrong; name what is missing.\nJSON: {"matches": true|false, "reason": "one short sentence (name any missing item)"}`,
        },
      ],
    });
  } catch (e) {
...
async function resolveCompletedTickets(sites) {
  ...
  for (const t of all) {
    ...
    if (stillOpen) {
      // still queued, but has the agent quietly given up retrying it (see
      // REVISION_FAIL_LIMIT in lib/agent.js)? Surface that instead of
      // leaving the ticket's countdown running forever on something that
      // will never resolve itself.
      const gaveUp = await store.get(`agent:revgaveup:${t.todoId}`).catch(() => null);
      if (gaveUp) {
        t.status = 'needs attention';
        t.verify = {
          verified: false,
          note: t.lastAttempt?.reason
            ? `Stopped retrying — ${t.lastAttempt.reason}`
            : 'The agent tried and failed several times in a row and stopped retrying automatically — needs a manual look.',
          checkedAt: Date.now(),
        };
        changed = true;
      }
    ...
    if (shipEntry) {
      ...
      t.shipFiles = shipEntry.files || t.shipFiles || null;
      // the host needs a few minutes to publish a commit; checking sooner tests the OLD page
      if (shipEntry.at && Date.now() - shipEntry.at < Number(process.env.REVISION_VERIFY_WAIT_MS ?? 240000)) continue;
    }
    ...
    if (verify.verified === false) {
      ...
      t.status = 'needs attention';
      changed = true;
      await askOwner({ kind: 'blocked-revision', text: `${site.name}: a fix shipped for "${String(t.summary || t.subject).slice(0, 80)}" but the live site does not look right (${String(verify.note || '').slice(0, 90)}).`, payload: { ticketId: t.id }, choices: 'Reply DONE if it is fine, RETRY to redo it, SKIP to drop it' }).catch(() => {});
      continue;
    }

    const mail = await sendRevisionCompletedEmail(site, t);
    await notifyOwner(`Done: ${site.name} - "${String(t.summary || t.subject).slice(0, 90)}" is live. Client ${mail.sent ? 'emailed' : 'NOT emailed (' + (mail.reason || 'unknown') + ')'}. ${site.url}`, { subject: `Revision done: ${site.name}` }).catch(() => {});
    t.status = 'done';
    t.doneAt = Date.now();
...
async function recordAttempt(site, result) {
  const all = await readArr('revisions:all');
  // the ticket the agent actually worked (its to-do id), falling back to the
  // first queued one for the site if the cycle didn't say. Attaching to the
  // wrong ticket meant a "can't do this" question about ticket B was sent
  // about ticket A — and a DONE reply would have closed the wrong client's.
  const t =
    (result?.todoId && all.find((x) => x.todoId === result.todoId && x.status === 'scheduled')) ||
    all.find((x) => x.slug === site.slug && x.status === 'scheduled');
  if (!t) return;
  const outcome =
    result?.action === 'change'
      ...
      ? 'error'
      : 'no change made';
  t.lastAttempt = { at: Date.now(), outcome, reason: String(result?.reason || result?.error || '').slice(0, 500) || null };
  await store.set('revisions:all', JSON.stringify(all));
  if (result?.blocked) await notifyBlocked(site, t, result.reason || result.error || '').catch(() => {});
}

// Per direct ask: don't make this wait for the daily digest — the moment
// the agent hits something it genuinely can't do via a file edit (live
// database state, a third-party dashboard setting, etc.), Mondo should hear
// about it right then, with exactly what and why. One-time per ticket
// (blockedNotified:<id>) so a still-shipped partial fix or a permanently
// given-up ticket doesn't re-send the same email on every later poll.
async function notifyBlocked(site, ticket, reason) {
  const key = `blockedNotified:${ticket.id}`;
  if (await store.get(key).catch(() => null)) return;
  await store.set(key, '1', { ex: 60 * 60 * 24 * 60 }).catch(() => {});
  await askOwner({
    kind: 'blocked-revision',
    text: `${site.name}: I can't do "${String(ticket.summary || ticket.subject).slice(0, 80)}" by editing files. ${String(reason).slice(0, 110)}`,
    payload: { ticketId: ticket.id },
    choices: 'Reply DONE once you handled it, RETRY after giving access, SKIP to drop it',
  }).catch(() => {});
}
...
async function workOnePendingRevision(sites, deadline) {
  ...
  for (const site of sites) {
    ...
    await recordAttempt(site, result).catch(() => {});
...
export async function checkRevisionInbox({ maxMs = 50000, rescan = null } = {}) {
  ...
  try {
    let requeued = 0;
    if (rescan) requeued = await unlabelProcessed(rescan.q || '', { days: rescan.days || 14 }).catch(() => 0);
    const out = await runInboxCheck(maxMs, { sinceDays: rescan ? rescan.days || 14 : 0 });
    return rescan ? { ...out, requeued } : out;
  } finally {
    await store.set('revisions:lock', '', { ex: 1 }).catch(() => {});
  }
}

// What the agent works from: the client's whole email (the 200-character summary alone
// dropped every date, price and link) plus the text of any attached file.
async function saveTicketContent(msg) {
  const text = `--- the client's email (from ${msg.from}, subject "${msg.subject}") ---\n${msg.body || msg.snippet || ''}${msg.attachText ? '\n\n' + msg.attachText : ''}${msg.attachNotes && msg.attachNotes.length ? '\n\n(Attachments that could not be read: ' + msg.attachNotes.join('; ') + ')' : ''}`;
  await store.set(`revisions:attach:${msg.id}`, text.slice(0, 30000), { ex: 60 * 60 * 24 * 60 }).catch(() => {});
}
async function runInboxCheck(maxMs, { sinceDays = 0 } = {}) {
  const t0 = Date.now();
  ...
  const since = dateQuery(sinceDays ? new Date(Date.now() - sinceDays * 86400000) : new Date(lastCheckSec * 1000 - 86400000)); // 1 day of slack so day-boundary emails aren't missed
  ...
  for (const msg of mail) {
    ...
    // known automated senders (meeting-notes bots, notification systems)
    // never send a specific personal request — skip the classifier
    // entirely rather than risk it reading too much into report content.
    if (looksAutomated(msg.from)) {
      await labelProcessed(msg.id).catch(() => {});
      continue;
    }

    const trust = await senderTrust(msg.from, sites);
    if (msg.attachments && msg.attachments.length) {
      const got = await extractAttachmentText(msg.attachments).catch((e) => ({ text: '', notes: ['attachments could not be read: ' + String(e.message || e).slice(0, 80)] }));
      msg.attachText = got.text;
      msg.attachNotes = got.notes;
    }
    let cls;
    try {
      cls = await classify(msg, sites, trust);
    } catch (e) {
    ...
    // someone answering / continuing a conversation we're already part of
    // ("thanks for sending the site!") is never a fresh request — leave it
    // alone for Mondo to read himself, no reply, no ticket.
    if (!trust.owner && (await threadHasSentMessage(msg.threadId))) {
      await labelProcessed(msg.id).catch(() => {});
      continue;
    }
    ...
    // A verified client always resolves to a site (the model's pick if it
    // named one, else the site their address belongs to). An unknown sender
    // only resolves if the model matched a site — and even then it only ever
    // becomes a HELD ticket below, never a reply. No match at all: drop it.
    const site = sites.find((s) => s.slug === cls.slug) || (trust.trusted ? sites.find((s) => s.slug === trust.slug) : null) || null;
    const replyTo = emailOf(msg.from);
    const wasAlreadyKnown = trust.trusted;
    // a genuine request we could not match to a site is NOT dropped any more — it becomes a
    // "needs attention" ticket (with the attachment text saved) and the owner is told
    const orphan = !site;
    // The model flags its own uncertainty (cls.confident) — this used to be
    // computed and then completely ignored, so a genuinely unsure guess got
    // the exact same treatment as a certain one: auto-reply to whoever sent
    // it promising "we'll have it updated in 3 business days," a calendar
    // hold, the works. That's the real risk with a plain-English classifier
    // reading a general inbox — not missing the word "revision" in the
    // subject (it was never looking for that), but confidently acting on a
    // borderline read of something that might be spam, a vendor pitch, or an
    // unrelated question. Low-confidence hits now stop here: a ticket is
    // still logged so nothing silently vanishes, but no reply goes out, no
    // calendar hold is made, and nothing is queued for the agent — it sits
    // as "needs attention" for a human decision.
    // ...and an unverified sender is ALWAYS held, whatever the model thinks.
    const lowConfidence = orphan || cls.confident === false || !trust.trusted;
    ...
    if (!lowConfidence) {
      ...
      // queue it for the agent — this is what actually gets it fixed, not
      // just acknowledged. Sites we couldn't match to a client can't be
      // auto-queued; those stay "needs attention" for Mondo to route by hand.
      if (site) {
        todoId = await addRevisionTodo(site.slug, { title: cls.summary, detail: `Requested by ${msg.from} — the client's full email${msg.attachText ? ' and attached file text' : ''} is provided below`, ticketId: msg.id }).catch(() => null);
      }
    }

    await saveTicketContent(msg);
    const ticket = {
      id: msg.id,
      threadId: msg.threadId || null,
      slug: site?.slug || null,
      siteName: site?.name || 'Unmatched site',
      hasAttachment: !!(msg.attachments && msg.attachments.length),
      attachNotes: msg.attachNotes || [],
      from: msg.from,
      subject: msg.subject,
      summary: cls.summary,
      receivedAt: Date.now(),
    ...
    // confirmation email — once they're known, no more of these, ever. Only
    // for actions actually taken; nothing to confirm on a low-confidence hit.
    if (site && !wasAlreadyKnown && !lowConfidence && !trust.owner) await sendGuessConfirmation(ticket, site);

    // Tell Mondo by text. A verified client is just an FYI; an unverified
    // sender (new address, changed email, a meeting contact) becomes a yes/no
    // — YES replies to them + queues it AND remembers the address so this
    // never needs asking again, NO drops it. Never blocks the pipeline.
    const who = String(msg.from || '').replace(/<.*>/, '').replace(/"/g, '').trim() || emailOf(msg.from);
    if (orphan) {
      await notifyOwner(`A change request from ${who} (${emailOf(msg.from)}) could not be matched to any site: "${cls.summary}". Open Revisions in the dashboard and assign it to a site — nothing was sent to them.${msg.attachNotes && msg.attachNotes.length ? ' Note: ' + msg.attachNotes.join('; ') + '.' : ''}`, { subject: 'Revision request needs a site' }).catch(() => {});
    } else if (lowConfidence) {
      await askOwner({
        kind: 'unknown-sender',
        text: `${who} (${emailOf(msg.from)}) emailed a change for ${site.name}: "${cls.summary}". ${trust.trusted ? 'Not sure it is a real request.' : 'Not a known client address.'} Handle it and reply to them?`,
        payload: { ticketId: ticket.id, slug: site.slug },
      }).catch(() => {});
    } else {
      await notifyOwner(`New request from ${who} for ${site.name}: "${cls.summary}". Replied to them and queued it.`, { subject: `Revision request: ${site.name}` }).catch(() => {});
    }
  }
...
export async function assignTicketToSite(ticketId, slug) {
  ...
  const wasLowConfidence = t.lowConfidence === true;
  t.slug = slug;
  ...
  // a low-confidence ticket never got the normal reply/calendar hold when it
  // came in (see checkRevisionInbox) — confirming it here IS the human
  // saying "yes, this is real," so give it that same treatment now instead
  // of leaving the sender with silence and no calendar hold ever appearing.
  if (wasLowConfidence && t.todoId) {
    const replyTo = emailOf(t.from);
    const existingTickets = all.filter((x) => x.id !== t.id);
    const aheadInQueue = existingTickets.filter((x) => x.status === 'scheduled').length;
    const holdAt = new Date(nextCheckBoundaryMs() + aheadInQueue * CHECK_INTERVAL_MS);
    t.targetAt = holdAt.getTime();
    try {
      const ev = await createCalendarEvent({
        title: `Auto website revision update — ${site.name}`,
        description: `• What: ${t.summary}\n• When: auto-scheduled for the agent's next chance to work it. This moves automatically if the queue changes.\n\nFrom: ${t.from}\nSubject: ${t.subject}`,
        startAt: holdAt,
        durationMinutes: 10,
      });
      t.calendarLink = ev.htmlLink;
    ...
    try {
      await sendGmailReply({
        ...
        subject: t.subject,
        threadId: t.threadId,
        body: `Hi,\n\nThanks for sending this over — got it: ${t.summary}\n\nWe'll have it updated and live within ${PROMISE_BUSINESS_DAYS} business days, and I'll follow up personally once it's done.\n\n— ${process.env.REPORT_SIGNATURE || 'Inspiring Websites'}`,
      });
      t.repliedAt = Date.now();
    } catch (e) {
      /* logged nowhere specific — the ticket's repliedAt just stays unset */
    }
    t.lowConfidence = false;
  }
  await store.set('revisions:all', JSON.stringify(all));
...
// Re-run the QA check on a ticket that was flagged (e.g. before the checker was fixed) and,
// if the live page now matches, finish it the normal way: client email, owner note, done.
// files: the pages that changed, when the ticket predates the ship log recording them.
export async function recheckTicket(ticketId, { files = [] } = {}) {
  const all = await readArr('revisions:all');
  const t = all.find((x) => x.id === ticketId);
  if (!t || !t.slug) return { ok: false, error: 'ticket not found or has no site' };
  const sites = await listSites();
  const site = sites.find((s) => s.slug === t.slug);
  if (!site) return { ok: false, error: 'site not found' };
  if (files.length) t.shipFiles = files;
  const v = await verifyShippedFix(site, t);
  t.verify = { verified: v.verified, note: v.note, checkedAt: Date.now() };
  if (v.verified === false) {
    t.status = 'needs attention';
    await store.set('revisions:all', JSON.stringify(all));
    return { ok: true, verified: false, note: v.note };
  }
  const mail = await sendRevisionCompletedEmail(site, t);
  await notifyOwner(`Done: ${site.name} - "${String(t.shipSummary || t.summary || t.subject).slice(0, 90)}" is live. Client ${mail.sent ? 'emailed' : 'NOT emailed (' + (mail.reason || 'unknown') + ')'}. ${site.url}`, { subject: `Revision done: ${site.name}` }).catch(() => {});
  t.status = 'done';
  t.doneAt = Date.now();
  t.completionEmail = mail;
  await store.set('revisions:all', JSON.stringify(all));
  return { ok: true, verified: v.verified, note: v.note, mail };
}
...
// The "Retry" button — for a "needs attention" ticket where the underlying
// bug has since been fixed. Without this there was no way back: once the
// agent gives up on a ticket (agent:revgaveup:<todoId>), the give-up filter
// in runAgentCycleInner permanently excludes it from ever being picked up
// again, even after a real code fix ships — the only path back to "working"
// was cancelling and having the client re-send the exact same email.
export async function retryTicket(ticketId) {
  const all = await readArr('revisions:all');
  const t = all.find((x) => x.id === ticketId);
  if (!t) return { ok: false, error: 'ticket not found' };
  if (!t.slug) return { ok: false, error: 'ticket has no linked site' };
  if (!(await store.get(`revisions:attach:${t.id}`).catch(() => null))) {
    try {
      const m = await getMessageById(t.id);
      const got = m.attachments && m.attachments.length ? await extractAttachmentText(m.attachments).catch(() => ({ text: '', notes: [] })) : { text: '', notes: [] };
      m.attachText = got.text;
      m.attachNotes = got.notes;
      await saveTicketContent(m);
    } catch {
      /* the original email may be gone; the summary still goes */
    }
  }
  if (t.todoId) {
    await store.set(`agent:revgaveup:${t.todoId}`, '', { ex: 1 }).catch(() => {});
    await store.set(`agent:revfail:${t.todoId}`, '', { ex: 1 }).catch(() => {});
  }
  // a retry that blocks again is news again — allow a fresh question
  await store.set(`blockedNotified:${t.id}`, '', { ex: 1 }).catch(() => {});
  // covers both stuck cases: the to-do is still queued (agent gave up but
  // never removed it) — flags cleared above are enough — or it was already
  // shipped and removed but a QA check flagged the live site as not
  // matching, in which case there's nothing left for a cycle to pick up
  // until it's re-queued.
  const { current } = await todosState({ slug: t.slug }).catch(() => ({ current: null }));
  const stillThere = t.todoId && (current?.items || []).some((it) => it.id === t.todoId);
  if (!stillThere) {
    t.todoId = await addRevisionTodo(t.slug, { title: t.summary || t.subject || 'Revision', detail: `Requested by ${t.from}`, ticketId: t.id }).catch(() => t.todoId);
  }
  t.status = 'scheduled';
  t.verify = null;
  await store.set('revisions:all', JSON.stringify(all));
