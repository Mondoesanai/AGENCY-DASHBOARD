...
import { listNewMail, threadHasSentMessage, labelProcessed, createCalendarEvent, updateCalendarEvent, deleteCalendarEvent, sendGmailReply, sendGmailMessage, getMyEmailAddress, googleConfigured } from './google.js';
...
Sender verification (checked in code, not by you): ${trust && trust.trusted ? `this sender IS a verified client of ${trust.slug} (${trust.why}).` : `this sender is NOT a verified client — treat them as an unknown person: a meeting contact, friend, prospect, vendor, or someone replying to an email the agency owner sent.`}
...
async function runInboxCheck(maxMs) {
  ...
  for (const msg of mail) {
    ...
    if (await threadHasSentMessage(msg.threadId)) {
      await labelProcessed(msg.id).catch(() => {});
      continue;
    }
    ...
    const lowConfidence = cls.confident === false || !trust.trusted;
    ...
    if (site && !wasAlreadyKnown && !lowConfidence) await sendGuessConfirmation(ticket, site);
