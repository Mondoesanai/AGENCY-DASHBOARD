...
import { listNewMail, labelProcessed, createCalendarEvent, deleteCalendarEvent, sendGmailReply, sendGmailMessage, getMyEmailAddress, googleConfigured } from './google.js';
...
async function runInboxCheck(maxMs) {
  ...
  for (const msg of mail) {
    ...
    // check. Also match on threadId, not just the exact message id — Gmail
    // will sometimes group same-subject messages between the same two people
    // into one conversation even without explicit reply headers, so "the same
    // request" can legitimately show up under a message id we haven't seen
    // before even though it's the same thread we already ticketed.
    const already = (await readArr('revisions:all')).find((t) => t.id === msg.id || (msg.threadId && t.threadId === msg.threadId));
    if (already) {
      await labelProcessed(msg.id).catch(() => {});
      continue;
    }
