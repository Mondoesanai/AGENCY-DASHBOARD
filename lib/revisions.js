...
import { listNewMail, unlabelProcessed, threadHasSentMessage, labelProcessed, createCalendarEvent, updateCalendarEvent, deleteCalendarEvent, sendGmailReply, sendGmailMessage, getMyEmailAddress, googleConfigured } from './google.js';
...
const ownerEmails = () => [...new Set([...String(process.env.OWNER_EMAIL || '').split(/[,;\s]+/), 'mondoesanai@gmail.com'].map((x) => x.trim().toLowerCase()).filter(Boolean))];
...
async function runInboxCheck(maxMs, { sinceDays = 0 } = {}) {
  ...
  for (const msg of mail) {
    ...
    if (!lowConfidence) {
      ...
      if (site) {
        todoId = await addRevisionTodo(site.slug, { title: cls.summary, detail: `Requested by ${msg.from}${msg.attachText ? ' — the email has attached file(s); their full text is provided below' : ''}`, ticketId: msg.id }).catch(() => null);
      }
    }

    if (msg.attachText) await store.set(`revisions:attach:${msg.id}`, msg.attachText.slice(0, 30000), { ex: 60 * 60 * 24 * 60 }).catch(() => {});
    const ticket = {
      id: msg.id,
