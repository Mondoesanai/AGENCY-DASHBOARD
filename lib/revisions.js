...
async function notifyBlocked(site, ticket, reason) {
  ...
  if (!process.env.RESEND_API_KEY || !process.env.REPORT_FROM) return;
  ...
  const resend = new Resend(process.env.RESEND_API_KEY);
  const subject = `Can't do this one automatically — ${site.name}`;
  const body =
    `Hi,\n\nA client revision on ${site.name} hit something I can't do through a code change:\n\n` +
    `"${ticket.summary || ticket.subject}"\n\n${reason}\n\n` +
    `Anything else in that same request that WAS a real file edit has already shipped. This one needs you — ` +
    `once it's handled (or if there's a different way you'd like it done), use Retry on the ticket to give it another pass.\n\n` +
    `— sent automatically the moment this came up, not on the daily digest`;
  try {
    await resend.emails.send({ from: process.env.REPORT_FROM, to, subject, text: body });
  } catch {
    /* best-effort */
  }
}
