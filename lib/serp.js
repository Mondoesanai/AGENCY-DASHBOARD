...
export async function rankBatch({ domain, keywords, locationName, locationCode, device = 'mobile', concurrency = 4 }) {
  ...
  async function worker() {
    while (i < list.length) {
      const idx = i++;
      const kw = list[idx];
      const r = await serpRank({ keyword: kw, domain, locationName, locationCode, device }).catch((e) => ({ ok: false, error: String(e.message || e) }));
      if (!r.ok) {
        out[idx] = { keyword: kw, error: r.error };
        continue;
      }
      cost += r.cost || 0;
      out[idx] = { keyword: kw, rank: r.rank, url: r.url, found: r.found };
