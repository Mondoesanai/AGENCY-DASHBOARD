...
import { mergePullRequest, closePullRequest } from './github.js';

async function addChangelog(slug, text) {
...
export async function handleInbound({ from, body }) {
  ...
  try {
    ...
    } else if (ask.kind === 'new-page') {
      if (cmd === 'yes') {
        await mergePullRequest(p.repo, p.number);
        await addChangelog(p.slug, p.summary);
        reply = 'Published. It goes live in about a minute when the site redeploys.';
      } else if (cmd === 'no' || cmd === 'skip') {
        await closePullRequest(p.repo, p.number);
        reply = 'Discarded. Nothing was published.';
      }
