...
const STOP = new Set([
  'button', 'buttons', 'click', 'clicked', 'clicks', 'link', 'links', 'pressed', 'press', 'submit',
  'submitted', 'form', 'forms', 'cta', 'now', 'page', 'home', 'the', 'tap', 'tapped', 'when', 'someone',
]);
const toks = (s) =>
...
export async function siteStats(slug, conversionEvents = []) {
  ...
  // fuzzy: an event counts if it exactly matches, contains / is contained by an
  // entry, or shares a meaningful word — so "RSVP button pressed" still catches
  // an event named "rsvp" or "rsvp-click".
  const isConv = (name) => {
    if (!counts.length) return true;
    const n = String(name).toLowerCase();
    const nt = toks(n);
    return counts.some(
      (c) => n === c || n.includes(c) || c.includes(n) || toks(c).some((t) => nt.includes(t))
    );
  };
  ...
  // pageviews + device split + engaged-time (cheap mget across all 60 days)
  const pvKeys = days.map((d) => `${p}:day:${d}:pv`);
  const mobKeys = days.map((d) => `${p}:day:${d}:mobile`);
  const deskKeys = days.map((d) => `${p}:day:${d}:desktop`);
  const durSumKeys = days.map((d) => `${p}:day:${d}:dursum`);
  const durCntKeys = days.map((d) => `${p}:day:${d}:durcnt`);
  const [pvVals, mobVals, deskVals, durSumVals, durCntVals] = await Promise.all([
    store.mget(pvKeys),
    store.mget(mobKeys),
    store.mget(deskKeys),
    store.mget(durSumKeys),
    store.mget(durCntKeys),
  ]);
  const durSumCur = cur.reduce((t, d) => t + n(durSumVals[days.indexOf(d)]), 0);
  const durCntCur = cur.reduce((t, d) => t + n(durCntVals[days.indexOf(d)]), 0);
  const avgDwell = durCntCur ? Math.round(durSumCur / durCntCur) : null;
  const durSumPrev = prev.reduce((t, d) => t + n(durSumVals[days.indexOf(d)]), 0);
  const durCntPrev = prev.reduce((t, d) => t + n(durCntVals[days.indexOf(d)]), 0);
  const avgDwellPrev = durCntPrev ? Math.round(durSumPrev / durCntPrev) : null;
  ...
  const [paths, refs, events, leadSources] = await Promise.all([merge('paths'), merge('refs'), merge('events'), merge('leadsrc')]);
  ...
  return {
    ...
    avgDwell, // avg engaged seconds per page view, last 30d
    avgDwellPrev,
    trend,
    ...
    // which channel the actual conversion clicks came from — auto-tagged
    // from UTM params (utm_source/utm_medium) when present, falling back to
    // referrer classification otherwise. Distinct from "sources" above,
    // which is just pageview traffic, not enquiries.
    leadSources: leadSources.slice(0, 6),
  };
}
