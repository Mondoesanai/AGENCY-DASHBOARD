...
// Things the BUSINESS OWNER can do this month — no website work involved.
// This is the "Client suggestions" box + the "on your end" part of the email.
export function clientActions(audit, stats) {
  const out = [];
  const add = (title, why) => out.push({ title, why });
  ...
  const searchLight = !totalRef || !sources.find((x) => x.member === 'google') || (direct && direct.score / (totalRef || 1) > 0.7);

  add('Ask 3 recent happy customers for a Google review', 'Reviews are the single biggest thing you can do for local ranking and trust — send them the link this week.');

  if (searchLight)
    add('Add your website link to your Google Business Profile', 'Free, takes two minutes, and it feeds you steady local search traffic.');

  add('Put the link in your email signature and text replies', 'Every message you already send becomes a way for people to find and share your site.');

  add('Post the link on Facebook / Instagram with a recent job photo', 'One post a month with a real photo keeps new visitors coming.');

  if (hasData && stats.conversions === 0)
    add('Reply to new enquiries within 5–10 minutes', 'Speed wins the job — the first business to call back usually gets it.');
  else
    add('When someone contacts you, ask how they found you', 'Tells us what\'s working so we can do more of it.');

  add('Share it in local community / neighbourhood Facebook groups', 'Where the rules allow it — that\'s where nearby customers are looking.');

  add('Add the website to business cards, invoices and any signage', 'Cheap to do once, works forever.');

  if (hasData && stats.deltas && stats.deltas.visitors >= 15)
    add('You\'ve got momentum — keep the monthly photo post going', 'Traffic is climbing; consistency is what compounds it.');
