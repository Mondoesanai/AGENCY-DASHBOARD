  ...
  if (Date.now() - t0 < 30000) {
    ...
    const info = await Promise.all(
      sites.map(async (x) => {
        ...
        return { x, kw: Array.isArray(kw) ? kw.length : 0, at };
      })
    );
    const stale = info.filter((i) => i.kw > 0 && Date.now() - i.at > 3 * 24 * 3600000).sort((p, q) => p.at - q.at);
    const needsKeywords = info.filter((i) => i.kw === 0 && i.x.seoAgent !== false);
    let work;
    if (stale.length) {
  ...
  if (Date.now() - t0 < 18000) {
    try {
      ...
      for (const s of sites) {
        ...
        if (rep && rep.progress) continue;
        if (await store.get(`report:regen:${s.slug}:${MK}`).catch(() => null)) continue;
        await store.set(`report:regen:${s.slug}:${MK}`, '1', { ex: 60 * 60 * 24 * 40 }).catch(() => {});
