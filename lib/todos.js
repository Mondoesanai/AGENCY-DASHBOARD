...
// Drop a client-requested revision straight onto the to-do list, flagged so
// the agent's planning step treats it as the priority item next cycle
// (see lib/agent.js) — and so a biweekly refresh never silently drops it.
export async function addRevisionTodo(slug, { title, detail, ticketId }) {
  const raw = await store.get(`todos:${slug}`).catch(() => null);
  const current = raw ? (typeof raw === 'string' ? JSON.parse(raw) : raw) : { generatedAt: Date.now(), items: [] };
  const id = `rev-${ticketId}`;
  if ((current.items || []).some((x) => x.id === id)) return id; // already queued
  current.items = [{ id, title: title.slice(0, 140), detail: (detail || '').slice(0, 400), category: 'Revision', severity: 'high', source: 'revision', ticketId, addedAt: Date.now() }, ...(current.items || [])];
  await store.set(`todos:${slug}`, JSON.stringify(current));
  return id;
}

// Plain removal, no completion record — for undoing a queued-but-not-yet-
// shipped item (e.g. a wrong-site match caught before the agent got to it).
export async function removeTodo(slug, id) {
  const raw = await store.get(`todos:${slug}`).catch(() => null);
  const current = raw ? (typeof raw === 'string' ? JSON.parse(raw) : raw) : null;
  if (!current) return false;
  const before = (current.items || []).length;
  current.items = (current.items || []).filter((x) => x.id !== id);
  if (current.items.length === before) return false;
  await store.set(`todos:${slug}`, JSON.stringify(current));
  return true;
}

// The agent calls this once it's actually shipped a to-do item — removes it
// from the open list and logs it as completed (same log the monthly report
// pulls "what we did" from).
export async function completeTodo(slug, id) {
  const raw = await store.get(`todos:${slug}`).catch(() => null);
  const current = raw ? (typeof raw === 'string' ? JSON.parse(raw) : raw) : null;
  ...
  current.items = current.items.filter((x) => x.id !== id);
  await store.set(`todos:${slug}`, JSON.stringify(current));
  const log = await readArr(`todos:completed:${slug}`);
  log.unshift({ ...item, addressedAt: Date.now() });
  await store.set(`todos:completed:${slug}`, JSON.stringify(log.slice(0, 40)));
  return item;
}
...
Give the 5-7 highest-impact to-dos, mixing SEO and conversion-rate items, for a builder/agency to actually work through. JSON: {"items":[{"title":"short specific action","detail":"the evidence + why it matters, one or two sentences","category":"SEO"|"Conversion","severity":"high"|"med"|"low"}]}`,
  ...
  // a client-requested revision never gets silently dropped by a biweekly
  // regen — it stays on the list, still first, until the agent actually
  // ships it (completeTodo()).
  const stillOpenRevisions = (prev?.items || []).filter((x) => x.source === 'revision');
  const finalItems = [...stillOpenRevisions, ...items];

  // an item that was open last time and isn't anymore (and wasn't a pending
  // revision) reads as "addressed" — soft language on purpose: we know it's
  // no longer on the list, we don't claim to know exactly why, and we never
  // attach a made-up improvement %.
  const newIds = new Set(finalItems.map((x) => x.id));
  const addressed = (prev?.items || []).filter((x) => x.source !== 'revision' && !newIds.has(x.id));
  if (addressed.length) {
  ...
  const record = { generatedAt: Date.now(), items: finalItems };
  await store.set(`todos:${site.slug}`, JSON.stringify(record));
  return { ok: true, items: finalItems, addressed, cost: usd };
}
