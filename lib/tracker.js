...
(function () {
  ...
  // UTM-based source, captured once on the landing page and kept for the
  // rest of the session (sessionStorage) so a conversion three pages later
  // still gets attributed to how the visitor actually arrived, not "direct".
  // Falls back to referrer-based classification server-side (unchanged)
  // when there's no UTM to read.
  function getSource() {
    try {
      var stored = sessionStorage.getItem("iw_src");
    ...
    try {
      ...
      if (src) val = (src + (med ? "-" + med : "")).toLowerCase().replace(/[^a-z0-9-]+/g, "-").slice(0, 40);
    } catch (e) {}
    try { if (val) sessionStorage.setItem("iw_src", val); } catch (e) {}
    return val;
  }

  function send(type, name, path, dur) {
    var body = JSON.stringify({
      s: site, e: type, n: name || "",
      p: path || location.pathname,
      r: document.referrer || "", w: window.innerWidth || 0,
      u: location.origin || "", d: dur || 0,
      src: getSource()
    });
  ...
  // engaged time on page: only counts while the tab is visible, capped at 30min
  var mark = Date.now(), engagedMs = 0;
  function acc() {
    if (document.visibilityState === "visible") engagedMs += Date.now() - mark;
    mark = Date.now();
  }
  var engInt = setInterval(acc, 5000);
  function flush() {
    acc();
    var secs = Math.min(1800, Math.round(engagedMs / 1000));
    engagedMs = 0;
    if (secs >= 3) send("dur", "", null, secs);
  }
  ...
  setInterval(function () {
    if (location.pathname !== lastPath) { flush(); lastPath = location.pathname; send("pv"); }
  }, 800);
