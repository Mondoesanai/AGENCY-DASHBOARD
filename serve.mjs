  ...
  if (path === '/api/revisions-status') {
    ...
    return res.end(JSON.stringify({
      ...
      status: {
        ...
        tickets: [
          { id: '1', threadId: 't1', slug: 'relax-tax', siteName: 'Relax Tax', from: 'Angelete May <angelete@example.com>', subject: 'Website update', summary: 'Remove the "cash & check only" line from the homepage', receivedAt: Date.now() - 3 * 3600000, repliedAt: Date.now() - 3 * 3600000 + 120000, calendarLink: 'https://calendar.google.com/', todoId: 'rev-1', status: 'scheduled', targetAt: Date.now() + 8 * 60000, dueBy: Date.now() + 2 * 86400000 },
          { id: '1b', threadId: 't1b', slug: 'apostello-detailing', siteName: 'Apostello Detailing', from: 'Shiloh <shiloh@example.com>', subject: 'Add a testimonial', summary: 'Add a new 5-star testimonial to the homepage', receivedAt: Date.now() - 2 * 3600000, repliedAt: Date.now() - 2 * 3600000 + 90000, calendarLink: 'https://calendar.google.com/', todoId: 'rev-1b', status: 'scheduled', dueBy: Date.now() + 2 * 86400000, lastAttempt: { at: Date.now() - 6 * 60000, outcome: 'not eligible yet', reason: 'no GitHub repo set (Settings → Automation)' } },
